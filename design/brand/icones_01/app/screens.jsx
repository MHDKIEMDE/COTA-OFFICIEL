    ...
    <div style={{ height: '100%', background: sBG, color: sINK, position: 'relative', fontFamily: sFONT.ui }}>
      <ScrollContent>
        ...
        <div style={{ padding: '18px 20px 22px' }}>
          <div style={{ fontFamily: sFONT.mono, fontSize: 10, letterSpacing: '0.18em', color: sACCENT, marginBottom: 10 }}>★ COUPON DU JOUR · 9:30</div>
          <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', background: sBG2, padding: 18, border: `1px solid ${sLINE}` }}>
            ...
            <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontFamily: sFONT.mono, fontSize: 10, color: sDIM, letterSpacing: '0.12em', marginTop: 4 }}>COTE COMBINÉE</div>
              </div>
        ...
        {/* Today */}
        <ContentRow title="TOUS LES MATCHES DU JOUR" more="6 →">
            ...
            {[
              ...
              <div key={n} style={{
                ...
                <div>
                  ...
                  <div style={{ fontFamily: sFONT.mono, fontSize: 9, color: sDIM, letterSpacing: '0.08em' }}>{c} pronos</div>
                </div>
              </div>
            ))}
          </div>
    ...
    <div style={{ height: '100%', background: sBG, color: sINK, position: 'relative', fontFamily: sFONT.ui }}>
      <ScrollContent paddingTop={0}>
        ...
        <div style={{ display: 'flex', gap: 18, padding: '14px 20px', borderBottom: `1px solid ${sLINE}`, background: sBG, position: 'sticky', top: 0, zIndex: 10 }}>
          {['ANALYSE', '9 CRITÈRES', 'COTES', 'H2H'].map((t, i) => (
        ...
        <div style={{ padding: '20px 20px 14px' }}>
          <div style={{ background: sBG2, border: `1px solid ${sLINE}`, borderRadius: 14, padding: 18, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, position: 'relative' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: sFONT.mono, fontSize: 10, color: sDIM, letterSpacing: '0.15em' }}>VERDICT IA</div>
...
// ──────────────────────────────────────────────────────────────────────────────
// 3. COUPON DU JOUR — the 3-pick combo, fully detailed
// ──────────────────────────────────────────────────────────────────────────────
function ScreenCoupon() {
  const C = window.COUPON;
  const M = window.MATCHES;
  const pickMatches = C.picks.map(p => M.find(m => m.id === p.matchId));
  return (
    <div style={{ height: '100%', background: sBG, color: sINK, position: 'relative', fontFamily: sFONT.ui }}>
      <ScrollContent>
        <AppHeader />
        ...
        <div style={{ padding: '6px 20px 14px' }}>
          <div style={{ fontFamily: sFONT.mono, fontSize: 10, color: sACCENT, letterSpacing: '0.18em' }}>COUPON · {C.date.toUpperCase()}</div>
          <div style={{ fontFamily: sFONT.title, fontSize: 32, letterSpacing: '-0.04em', marginTop: 6 }}>3 picks IA<br/>combinés.</div>
        </div>
        ...
        <div style={{ padding: '0 20px 20px' }}>
          <div style={{ background: sBG2, border: `1px solid ${sLINE}`, borderRadius: 16, padding: 20, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
              <div>
                <div style={{ fontFamily: sFONT.mono, fontSize: 10, color: sDIM, letterSpacing: '0.15em' }}>COTE COMBINÉE</div>
            ...
            <div style={{ display: 'flex', marginTop: 18, gap: 12, position: 'relative' }}>
              <div style={{ flex: 1, padding: '12px 14px', background: sBG, border: `1px solid ${sLINE2}`, borderRadius: 10 }}>
                <div style={{ fontFamily: sFONT.mono, fontSize: 9, color: sDIM, letterSpacing: '0.15em' }}>MISE</div>
              ...
              <div style={{ flex: 1, padding: '12px 14px', background: sBG, border: `1px solid ${sACCENT}`, borderRadius: 10 }}>
                <div style={{ fontFamily: sFONT.mono, fontSize: 9, color: sACCENT, letterSpacing: '0.15em' }}>GAIN POSSIBLE</div>
        ...
        {/* Picks */}
        <div style={{ padding: '0 20px 20px' }}>
          <div style={{ fontFamily: sFONT.mono, fontSize: 10, color: sDIM, letterSpacing: '0.18em', marginBottom: 10 }}>LES 3 PICKS</div>
...
function ScreenNotif() {
  const items = [
    { kind: 'coupon', t: '09:30',     d: 'COTA',    title: 'Coupon du jour disponible', sub: '3 picks combinés · @4.55 · confiance 87%', new: true },
    { kind: 'live',   t: 'il y a 12min', d: 'LIVE',  title: 'PSG-OM commence dans 30min', sub: 'Cote PSG passée de 1.72 → 1.65', new: true },
    { kind: 'win',    t: 'Hier 22:48', d: '✓ GAGNÉ', title: 'Coupon validé : +44.20€', sub: 'PSG · LIV · RMA — tous les picks gagnants' },
    { kind: 'win',    t: 'Hier 21:35', d: '✓ VALIDÉ', title: 'PSG–OM : Victoire PSG 2–1', sub: 'Pick @1.65 validé · gain individuel +€16.50' },
    { kind: 'live',   t: 'Hier 20:50', d: 'LIVE',  title: 'PSG–OM : but de Mbappé (62\')', sub: '2–1 PSG · pick sur la voie de la validation' },
    { kind: 'cote',   t: '17 mai 14:00', d: 'COTE',  title: 'Real–Bayern : BTTS passe à @1.55', sub: 'Cote favorable détectée par l\'IA' },
    { kind: 'loss',   t: '16 mai 23:10', d: '✗ PERDU', title: 'Coupon perdu : -10€', sub: 'OL–Lille : 1–1 alors que +2.5 attendu' },
  ];
    ...
    <div style={{ height: '100%', background: sBG, color: sINK, position: 'relative', fontFamily: sFONT.ui }}>
      <ScrollContent>
        ...
        <div style={{ padding: '0 20px 20px', display: 'flex', gap: 8 }}>
          ...
          <StatBlock value="47/59" label="PICKS GAGNANTS" />
