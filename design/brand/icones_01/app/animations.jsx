...
function AnimSplash() {
  const criteria = ['FORME', 'H2H', 'DOM/EXT', 'BLESSURES', 'MÉTÉO', 'COTES', 'CARTONS', 'POSSESS.', 'XG'];
  return (
    <div style={{
...
// ── 3 · COUPON VALIDÉ ─────────────────────────────────────────────────────────
// 3 picks slide in stacked, multiplier resolves, ✓ COUPON VALIDÉ stamps.
function AnimCouponValid() {
  const picks = [
    { label: 'PSG – OM',   type: 'VICTOIRE PSG', odds: 1.65 },
    { label: 'LIV – ARS',  type: '+2.5 BUTS',    odds: 1.78 },
    { label: 'RMA – BAY',  type: 'BTTS OUI',     odds: 1.55 },
  ];
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: aBG, color: aINK, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32, fontFamily: aFONT.ui }}>
      ...
      <div style={{ width: 360 }}>
        <div style={{ fontFamily: aFONT.mono, fontSize: 10, color: aACCENT, letterSpacing: '0.2em', marginBottom: 12 }}>COUPON DU JOUR</div>
        ...
        {stage >= 5 && (
          <div style={{
            ...
            <div style={{ fontFamily: aFONT.title, fontSize: 18, color: aBG, letterSpacing: '0.06em' }}>COUPON VALIDÉ</div>
          </div>
        )}
      </div>
    </div>
...
Object.assign(window, { AnimSplash, AnimConfidenceReveal, AnimCouponValid });
