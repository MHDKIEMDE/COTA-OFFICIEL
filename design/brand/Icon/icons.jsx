/* global React */
// COTA app icon — six variations, all built around a single capital C.
// Each variant is a self-contained SVG that scales cleanly from 16px to 1024px.
...
// ── Variant 01 — Frame --------------------------------------------------------
// The existing brand mark, simplified. Yellow ring frame, single C centered.
...
// ── Variant 02 — Bleed --------------------------------------------------------
// Large C bleeding past the edges, no frame, small accent square corner.
function IconBleed({ size = 120, radius = 0.18 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <defs>
        <clipPath id={`clip-bleed-${size}`}>
          <rect width={size} height={size} rx={r} />
        </clipPath>
      </defs>
      <g clipPath={`url(#clip-bleed-${size})`}>
        ...
        >C</text>
        {/* corner accent — echoes the wordmark terminal square */}
        <rect
          x={size - sq - size * 0.1} y={size - sq - size * 0.1}
          width={sq} height={sq} fill={ACCENT}
        />
      </g>
    </svg>
  );
...
// ── Variant 03 — Underscore ---------------------------------------------------
// C floats above the yellow underline bar from the wordmark.
function IconUnderscore({ size = 120, radius = 0.18 }) {
  const r = size * radius;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <defs>
        <clipPath id={`clip-us-${size}`}>
          <rect width={size} height={size} rx={r} />
        </clipPath>
      </defs>
      <g clipPath={`url(#clip-us-${size})`}>
        <rect width={size} height={size} fill={BG} />
        <text
          x={size / 2} y={size * 0.72}
          textAnchor="middle"
        ...
        >C</text>
        {/* yellow bar + terminal square */}
        <rect
          x={size * 0.18} y={size * 0.84}
          width={size * 0.58} height={size * 0.05}
          fill={ACCENT}
        />
        <rect
          x={size * 0.76} y={size * 0.84}
...
// ── Variant 04 — Ring ---------------------------------------------------------
// C nested inside a yellow ring (the "O" detail from the wordmark, around the C).
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <circle
        cx={size / 2} cy={size / 2}
        r={size * 0.42}
        fill="none" stroke={ACCENT} strokeWidth={size * 0.022}
      />
      <text
        x={size / 2} y={size * 0.69}
        textAnchor="middle"
        fontFamily='"Archivo Black", "Archivo", sans-serif'
        fontWeight="900"
        fontSize={size * 0.58}
...
// ── Variant 05 — Yellow inverse -----------------------------------------------
// Yellow C on dark, no frame. The most confident, the most jaune.
...
const FRAME_VARIANTS = [
  { id: 'frame-orig',  label: 'FRAME · origine',     Comp: IconFrame,      note: 'Tel quel — cadre fin, C centré.' },
  { id: 'frame-dot',   label: 'FRAME · live dot',    Comp: IconFrameDot,   note: 'Pastille jaune coin haut-droit — signal "live / cote ouverte".' },
  { id: 'frame-thick', label: 'FRAME · cadre épais', Comp: IconFrameThick, note: 'Accent jaune plus large, C qui pousse — plus de présence sur springboard.' },
  { id: 'frame-bar',   label: 'FRAME × UNDERSCORE',  Comp: IconFrameBar,   note: 'Cadre + barre jaune sous le C — le mix fidèle au wordmark.' },
  { id: 'frame-pulse', label: 'FRAME · pulse',       Comp: IconFramePulse, note: 'Double cadre concentrique — évoque le pulse du hero animé.' },
];

const VARIANTS = [
  { id: 'frame',      label: '01 · FRAME',      Comp: IconFrame,      note: 'Évolution directe de l\'icône actuelle — cadre jaune + C massif.' },
  { id: 'bleed',      label: '02 · BLEED',      Comp: IconBleed,      note: 'C qui déborde, sans cadre, carré jaune en coin (rappel du terminal du logo).' },
  { id: 'underscore', label: '03 · UNDERSCORE', Comp: IconUnderscore, note: 'C flottant au-dessus de la barre jaune du wordmark.' },
  { id: 'ring',       label: '04 · RING',       Comp: IconRing,       note: 'Anneau jaune autour du C — rappel du O du logo, en orbite.' },
  { id: 'yellow',     label: '05 · YELLOW',     Comp: IconYellow,     note: 'Inverse maximal : C jaune sur noir. Le plus signal.' },
  { id: 'notch',      label: '06 · NOTCH',      Comp: IconNotch,      note: 'C en négatif, découpé dans une plaque jaune. Plus iconique, moins lisible petit.' },
];
...
function App() {
  return (
    <DesignCanvas defaultLayout="grid">
      <DCSection id="frame-deep" title="FRAME — variations gaming" subtitle="Tu aimes le 1. Voici 4 manières de le muscler pour le contexte foot pronostic, plus un test parmi des icônes type bookmaker.">
