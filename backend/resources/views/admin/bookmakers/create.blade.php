...
<div class="max-w-2xl">
    <form action="{{ route('admin.bookmakers.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-dark-100 rounded-xl border border-gray-700/50 p-6 space-y-6">
            <!-- ID -->
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Identifiant (slug) *</label>
                <input type="text" name="id" value="{{ old('id') }}" required
                       ...
                       placeholder="ex: betwinner, 1xbet, melbet">
                <p class="text-xs text-gray-500 mt-1">Minuscules, chiffres, tirets et underscores uniquement</p>
            </div>
            ...
            <div>
                ...
                <input type="url" name="affiliate_url" value="{{ old('affiliate_url') }}" required
                       class="w-full bg-gray-800 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-primary"
                       placeholder="https://betwinner.com/ref/XXXXX">
            </div>
