@props(['pageTitle' => 'COTA — Espace Membre'])
<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
:root {
  ...
  --cool-dim:rgba(111,180,217,.12);--accent-dim:rgba(232,255,54,.1);
...
/* Sidebar */
.wd-sidebar{width:248px;flex-shrink:0;background:var(--bg2);border-right:1px solid var(--line);display:flex;flex-direction:column;padding:22px 16px;position:sticky;top:0;height:100vh;overflow-y:auto}
.wd-brand{display:flex;align-items:center;gap:11px;padding:0 6px;text-decoration:none}
.wd-brand-icon{width:30px;height:30px;border-radius:8px;background:var(--accent);display:flex;align-items:center;justify-content:center;flex-shrink:0}
.wd-brand-name{font-family:var(--title);font-size:19px;letter-spacing:-.02em;color:var(--ink);line-height:1}
.wd-brand-sub{font-family:var(--mono);font-size:8.5px;color:var(--dim);letter-spacing:.22em;margin-top:3px}

.wd-profile{display:flex;flex-direction:column;align-items:center;text-align:center;padding:22px 0 20px;margin-top:18px;border-top:1px solid var(--line);border-bottom:1px solid var(--line);text-decoration:none}
.wd-avatar{width:72px;height:72px;border-radius:50%;background:var(--accent);color:var(--bg);display:flex;align-items:center;justify-content:center;font-family:var(--title);font-size:26px;letter-spacing:-.02em;position:relative}
.wd-avatar-dot{position:absolute;bottom:2px;right:2px;width:16px;height:16px;border-radius:50%;background:var(--win);border:3px solid var(--bg2)}
...
.wd-profile-pill{margin-top:12px;display:inline-flex;align-items:center;gap:5px;background:var(--accent-dim);color:var(--accent);border-radius:999px;padding:4px 10px;font-family:var(--mono);font-size:10px;font-weight:700}
...
.wd-navitem{display:flex;align-items:center;gap:12px;padding:11px 13px;border-radius:10px;font-size:13.5px;color:var(--dim);text-decoration:none;letter-spacing:-.01em;transition:background .15s,color .15s;border:none;background:none;width:100%;text-align:left;cursor:pointer}
...
.wd-iconbtn{width:42px;height:42px;border-radius:11px;background:var(--bg2);border:1px solid var(--line);color:var(--ink);display:flex;align-items:center;justify-content:center;position:relative;cursor:pointer;text-decoration:none}
.wd-iconbtn:hover{border-color:var(--line2);color:var(--ink)}
.wd-dot{position:absolute;top:10px;right:11px;width:7px;height:7px;border-radius:4px;background:var(--accent)}
.wd-cta{display:inline-flex;align-items:center;gap:7px;background:var(--accent);color:var(--bg);border:none;border-radius:11px;padding:0 18px;height:42px;font-family:var(--title);font-size:12.5px;letter-spacing:.02em;cursor:pointer;white-space:nowrap;text-decoration:none}
.wd-cta:hover{opacity:.9;color:var(--bg)}
.wd-cta-block{width:100%;justify-content:center;height:46px}
.wd-ghost-btn{display:inline-flex;align-items:center;gap:7px;background:transparent;border:1px solid var(--line2);color:var(--ink);border-radius:10px;padding:9px 15px;font-family:var(--ui);font-size:12.5px;font-weight:600;cursor:pointer;white-space:nowrap;text-decoration:none}
...
.wd-search{flex:1.6;min-width:200px;display:flex;align-items:center;gap:10px;background:var(--bg2);border:1px solid var(--line);border-radius:12px;padding:0 16px}
.wd-search input{flex:1;background:transparent;border:none;outline:none;color:var(--ink);font-family:var(--ui);font-size:13.5px;padding:14px 0}
.wd-search input::placeholder{color:var(--dim2)}
.wd-filter{flex:1;min-width:130px;text-align:left;background:var(--bg2);border:1px solid var(--line);border-radius:12px;padding:9px 14px;cursor:pointer;display:flex;flex-direction:column}
...
.wd-filterbtn{background:var(--accent);color:var(--bg);border:none;border-radius:12px;padding:0 20px;font-family:var(--title);font-size:12.5px;display:flex;align-items:center;gap:7px;cursor:pointer;white-space:nowrap;text-decoration:none}
...
/* Pill */
.wd-pill{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:4px 10px;font-family:var(--mono);font-size:10.5px;font-weight:700}
...
/* Summary bars */
.wd-sum-item{margin-bottom:20px}.wd-sum-item:last-child{margin-bottom:0}
...
.wd-matchcard-back{height:132px;position:relative;overflow:hidden;background:linear-gradient(135deg,var(--bg3) 0%,var(--bg2) 100%)}
...
.wd-back-btn{display:inline-flex;align-items:center;gap:6px;background:none;border:none;color:var(--dim);font-family:var(--ui);font-size:12.5px;cursor:pointer;padding:0;margin-bottom:18px;text-decoration:none}
...
.wd-plan-btn{width:100%;height:44px;border-radius:11px;background:var(--bg3);border:1px solid var(--line2);color:var(--ink);font-family:var(--ui);font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;text-decoration:none}
.wd-plan-btn:hover:not(:disabled){border-color:var(--accent);color:var(--ink)}
.wd-plan-btn.cta{background:var(--accent);border:none;color:var(--bg);font-family:var(--title);letter-spacing:.02em}
.wd-plan-btn.cta:hover{opacity:.9;color:var(--bg)}
.wd-plan-btn.current{background:transparent;color:var(--dim);cursor:default;border-style:dashed}

/* Toggle */
.wd-toggle{width:42px;height:24px;border-radius:12px;background:var(--bg3);border:1px solid var(--line2);position:relative;flex-shrink:0;cursor:pointer;transition:background .15s,border-color .15s}
...
<div class="wd-shell">
  ...
  <aside class="wd-sidebar">
    <a href="{{ route('home') }}" class="wd-brand">
      <div class="wd-brand-icon">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
          <path d="M12 2L3 7l9 5 9-5-9-5zM3 12l9 5 9-5M3 17l9 5 9-5" stroke="#0b0d10" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </div>
      <div>
    ...
    <a href="{{ route('profile') }}" class="wd-profile">
      ...
      <div class="wd-profile-city">{{ auth()->user()->email ?? 'Membre COTA' }}</div>
    ...
    <form method="POST" action="{{ route('logout') }}" style="margin-top:6px">
      @csrf
      <button type="submit" class="wd-navitem wd-logout">
    ...
    @else
    <a href="{{ route('login') }}" class="wd-navitem wd-logout">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M14 4h-7v16h7M10 12h10m0 0-3-3m3 3-3 3" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>
      Connexion
    </a>
    @endauth
